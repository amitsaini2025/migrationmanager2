...
class ClientAccountsController extends Controller
{
    ...
    private function applyDateFilters($query, Request $request)
    {
        ...
        if ($dateFilterType && $dateFilterType !== 'custom' && $dateFilterType !== 'financial_year') {
            ...
            if (count($years) == 2) {
                $fyStartDate = Carbon::createFromDate($years[0], 7, 1)->startOfDay(); // July 1st
                $fyEndDate = Carbon::createFromDate($years[1], 6, 30)->endOfDay();   // June 30th
    ...
    public function uploadclientreceiptdocument(Request $request)
    {
        ...
        $client_info = Admin::select('client_id')->where('id', $id)->first();
        if (! empty($client_info)) {
        ...
        if ($request->hasfile('document_upload')) {
            ...
            if ($saved) {
                ...
                $fetchd = $uploadedDocumentId
                    ? Document::where('id', $uploadedDocumentId)->get()
                    : collect(); // Return empty collection if no document ID was stored
                ob_start();
                foreach ($fetchd as $fetch) {
                    $admin = Staff::where('id', $fetch->user_id)->first();
                    ?>
               <tr class="drow" id="id_<?php echo $fetch->id; ?>">
                   <td><div data-id="<?php echo $fetch->id; ?>" data-name="<?php echo $fetch->file_name; ?>" class="doc-row">
                       <?php echo IconHelper::fromLegacy('fas fa-file-image'); ?> <span><?php echo $fetch->file_name; ?><?php echo '.'.$fetch->filetype; ?></span>
                   </div></td>
        ...
        if (isset($request->clickedReceiptIds) && ! empty($request->clickedReceiptIds)) {
            ...
            if ($affectedRows > 0) {
                ...
                foreach ($request->clickedReceiptIds as $clickedKey => $clickedVal) {
                    ...
                    $client_info = Admin::select('client_id')->where('id', $invoice_info->client_id)->first();
        ...
        try {
            ...
            if ($hubdoc_status) {
                $hubdocSentAtFormatted = $hubdoc_status->hubdoc_sent_at
                    ? Carbon::parse($hubdoc_status->hubdoc_sent_at)->format('d/m/Y H:i')
                    : null;
    ...
    public function sendInvoiceToClient(Request $request, $id)
    {
        try {
            ...
            Mail::mailer()->to($clientEmail)->queue(new InvoiceEmailManager($invoiceArray));
