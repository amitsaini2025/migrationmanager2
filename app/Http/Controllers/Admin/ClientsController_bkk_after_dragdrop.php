    ...
    */
                ...
                if (!empty($requestData["{$type}_details"]) || !empty($requestData["{$type}_relationship_type"])) {
                    ...
                    foreach ($detailsArray as $key => $details) {
                        ...
                        $partnerData = [
                            ...
                            'gender' => $saveExtraFields ? $gender : null,
    ...
    public function getallclients(Request $request)
    {
        ...
        if ($squery != '') {
            ...
            if (!empty($matterClientIds)) {
                $matterClients = \App\Admin::whereIn('id', $matterClientIds)->get();
    ...
    public function getnotes(Request $request){
        ...
        foreach($notelist as $list){
            ...
            ?>
            <div class="note-card-redesign <?php if($list->pin == 1) echo 'pinned'; ?>" data-matterid="<?php echo $list->matter_id; ?>" id="note_id_<?php echo $list->id; ?>" data-type="<?php echo $typeLabel;?>">
                ...
                <div class="note-content-redesign"><?php echo nl2br(htmlspecialchars($desc, ENT_QUOTES, 'UTF-8'));?>
                    <?php //if($showReadMore){ ?>
                        <!--<span class="viewnote" data-id="<?php echo $list->id;?>">Read More</span>-->
                    <?php //} ?>
                </div>
    ...
    /**
     * Upload Personal Document with support for multiple files
     * This method handles both single and multiple file uploads for Personal Documents
     * Supports drag and drop functionality from frontend
     * 
     * @param Request $request - Contains file upload data and metadata
    ...
    public function uploadedudocument(Request $request) {
        ...
        // Get client information for file naming and storage
        $clientid = $request->clientid;
        $admin_info1 = \App\Admin::select('client_id', 'first_name')->where('id', $clientid)->first();
        $client_unique_id = !empty($admin_info1) ? $admin_info1->client_id : "";
        ...
        try {
            ...
            if ($request->hasfile('document_upload')) {
                ...
                foreach ($files as $file) {
                    ...
                    // Get checklist (doc category) for file naming
                    $req_file_id = $request->fileid;
                    $obj = \App\Document::find($req_file_id);
                    $checklistName = $obj->checklist;
                    
                    // Build unique file name: firstname_checklist_timestamp_uniqueid.ext
                    $name = $client_first_name . "_" . $checklistName . "_" . time() . "_" . uniqid() . "." . $extension;
    
                    // Define file path for S3 storage
                    $filePath = $client_unique_id . '/' . $doctype . '/' . $name;
                    ...
                    if ($obj) {
                        ...
                        // Log activity for client documents
                        if ($saved && $request->type == 'client') {
                            $subject = 'updated personal document';
                            $objs = new ActivitiesLog;
                            $objs->client_id = $clientid;
                ...
                // Set final response based on upload results
                if ($uploadedCount > 0) {
                    $response['status'] = true;
                    ...
                    // For backward compatibility, set single file response for first uploaded file
                    if (count($uploadedFiles) > 0) {
                        $firstFile = $uploadedFiles[0];
                        $response['filename'] = $firstFile['filename'];
                        $response['filetype'] = $firstFile['filetype'];
                        $response['fileurl'] = $firstFile['fileurl'];
                        $response['filekey'] = $firstFile['filekey'];
                        $response['doccategory'] = $firstFile['doccategory'];
                    }
                } else {
                    $response['message'] = 'No valid files were uploaded. Please check file names.';
                }
            } else {
...
//	echo '<pre>'; print_r($requestData); die;
    ...
    public function uploadAgreement(Request $request, $clientId)
    {
        ...
        $adminInfo = \App\Admin::select('client_id')->where('id', $clientId)->first();
