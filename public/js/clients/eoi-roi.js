                ...
                html += `
                    <div class="workflow-box info">
                        ...
                        <div class="workflow-detail">
                            ...
                            <span>${typeof formatDisplayDateTime === 'function' ? (formatDisplayDateTime(eoi.email_sent_at) || eoi.email_sent_at || 'N/A') : (eoi.email_sent_at || 'N/A')}</span>
                        </div>
                        <div class="workflow-actions">
                            <button type="button" class="btn btn-primary btn-sm" id="btn-resend-email" data-eoi-id="${eoi.id}">
                                ${typeof crmIconLegacy === 'function' ? crmIconLegacy('fas fa-envelope') : '<i class="fas fa-envelope"></i>'} Resend Email
            ...
            data.eoi_references.forEach(doc => {
                html += `<div class="form-check mb-2">
                            ...
                            <label class="form-check-label" for="doc-${doc.id}">
                                ...
                                <br><small class="text-muted">${doc.category} • ${doc.file_size_mb} MB • ${typeof formatDisplayDateTime === 'function' ? (formatDisplayDateTime(doc.created_at) || doc.created_at || '') : (doc.created_at || '')}</small>
                            </label>
                        </div>`;
                totalDocs++;
            });
