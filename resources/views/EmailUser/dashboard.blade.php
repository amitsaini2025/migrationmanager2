<x-app-layout>
    ...
    <style>
        ...
        /* Email Allocate Modal specific styling */
        .allocate-modal .select2-container {
            width: 100% !important;
        }
        
        .allocate-modal label {
        ...
        .allocate-modal .select2-selection__rendered {
            word-wrap: break-word;
            overflow-wrap: break-word;
            white-space: normal;
        }
    ...
    <div class="py-6" x-data="emailApp()" x-init="init()">
            ...
            <div class="bg-white border border-gray-200 sm:rounded-lg shadow-sm">
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 flex-1">
                            ...
                            <div class="min-w-0 flex-1 sm:max-w-xs">
                                ...
                                <select id="accountDropdown" x-model="selectedAccountId" @change="resetEmailPagination(); loadEmails()" 
                ...
                <div x-show="selectedEmails.length > 0" x-transition:enter="transition ease-out duration-200" 
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        ...
                        <div class="flex gap-2 flex-wrap">
                            <button @click="openAllocateModal()" 
                                    class="inline-flex items-center px-3 py-1.5 bg-purple-600 text-white rounded text-sm font-medium hover:bg-purple-700">
                                Allocate
                            </button>
                ...
                <div class="flex flex-col lg:flex-row h-[70vh]">
                    ...
                    <div class="lg:w-1/3 w-full border-r border-gray-200 overflow-y-auto bg-white"
                         @scroll="checkScrollForLoadMore($event)">
                        <!-- Loading Indicator -->
                        ...
                        <div x-show="!isLoading">
                                                ...
                                                <button type="button"
                                                    ...
                                                    <div class="flex items-start justify-between gap-3">
                                                        <div class="flex-1 min-w-0">
                                                            ...
                                                            <!-- Snippet -->
                                                            <p class="text-xs text-gray-500 line-clamp-2 leading-relaxed mb-2" x-text="email.snippet"></p>
                                                            
                                                            <!-- Client and Matter Info (permanently visible for allocated emails) -->
                                                            <div x-show="email.assign_client_name || email.assign_matter_name" class="flex flex-wrap gap-2 mb-1">
                                                                <span x-show="email.assign_client_name" class="inline-block px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-gray-800" x-text="'Client: ' + email.assign_client_name"></span>
                                                                <span x-show="email.assign_matter_name" class="inline-block px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-gray-800" x-text="'Matter: ' + email.assign_matter_name"></span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="flex flex-col items-end gap-2 flex-shrink-0">
                                                            ...
                                                            <div class="flex items-center gap-1">
                                                                ...
                                                                <button @click.stop="deleteEmail(email)" 
                                                                        class="p-1.5 hover:bg-red-100 rounded-full transition-colors opacity-0 group-hover:opacity-100"
                                                                        title="Delete email">
                            ...
                            <!-- Load More Indicator -->
                            <div x-show="isLoadingMore" class="p-4 text-center border-t border-gray-100">
                                <div class="inline-flex items-center px-4 py-2 text-sm text-gray-600">
                            ...
                            <div x-show="!hasMoreEmails && emails.length > 0" class="p-4 text-center border-t border-gray-100">
                                <p class="text-sm text-gray-500">No more emails to load</p>
                            </div>
            ...
            <div class="relative bg-white w-full max-w-4xl mx-4 rounded-lg shadow-lg max-h-[90vh] overflow-hidden">
                ...
                <div class="p-4 max-h-[calc(90vh-120px)] overflow-auto">
                    <div x-show="previewAttachment && previewAttachment.type && previewAttachment.type.startsWith('image/')" class="text-center">
                        <img :src="previewAttachment ? `/attachments/${previewAttachment.id}/view` : ''" :alt="previewAttachment ? (previewAttachment.name || previewAttachment.filename) : ''" class="max-w-full h-auto mx-auto" style="max-height: 70vh;">
                    </div>
                    <div x-show="previewAttachment && previewAttachment.type && previewAttachment.type.includes('pdf')" class="w-full">
                        <iframe :src="previewAttachment ? `/attachments/${previewAttachment.id}/view#toolbar=0&navpanes=0&scrollbar=0` : ''" class="w-full h-96 border-0" :title="previewAttachment ? (previewAttachment.name || previewAttachment.filename) : ''"></iframe>
                    </div>
                </div>
                <div class="border-t px-4 py-3 flex items-center justify-end space-x-3">
                    <button @click="previewAttachment ? downloadAttachment(previewAttachment) : null" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <a href="/email_users/signatures" target="_blank" class="text-blue-600 hover:text-blue-800 text-sm">Manage</a>
                            </div>
                        </div>
        ...
        <!-- Email Allocate Modal -->
        <div x-show="showAllocateModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
            <div class="absolute inset-0 bg-black bg-opacity-30" @click="closeAllocateModal()"></div>
            <div class="relative bg-white w-full max-w-lg mx-4 rounded-lg shadow-lg allocate-modal">
                <div class="border-b px-4 py-3 flex items-center justify-between">
                    <h3 class="text-lg font-semibold">Allocate Email</h3>
                    <button class="text-gray-500 hover:text-gray-700" @click="closeAllocateModal()">✕</button>
                </div>
                <div class="p-4 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Select Client</label>
                        <select id="clientSelect" 
                                class="mt-1 block w-full rounded-md border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" required>
                            <option value="">Select client...</option>
                            <template x-for="client in clients" :key="client.id">
                                <option :value="client.id" x-text="client.first_name + ' ' + client.last_name + ' (' + client.client_id + ')'"></option>
                            </template>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Select Matter</label>
                        <select id="matterSelect" 
                            ...
                            <template x-for="matter in clientMatters" :key="matter.id">
                                <option :value="matter.id" x-text="matter.client_unique_matter_no + ' - ' + matter.title"></option>
                            </template>
                        </select>
                    </div>
                </div>
                <div class="border-t px-4 py-3 flex justify-end space-x-2">
                    <button type="button" class="inline-flex items-center rounded-md border border-gray-300 text-gray-700 bg-white px-3 py-2 text-sm font-semibold hover:bg-gray-50" @click="closeAllocateModal()">Cancel</button>
                    <button type="button" class="inline-flex items-center rounded-md border border-purple-600 text-white bg-purple-600 px-3 py-2 text-sm font-semibold hover:bg-purple-700" @click="allocateEmails()">Allocate</button>
                </div>
            </div>
        </div>
        ...
        function emailApp() {
            return {
                ...
                loadEmails() {
                    ...
                    fetch(`/email_users/emails/sync/${this.selectedAccountId}?${params.toString()}`)
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                const newEmails = this.processEmails(data.emails);
                                
                                if (this.emailOffset === 0) {
                                    // First load - replace emails
                                    this.emails = newEmails;
                                } else {
                                    // Infinite scroll - append emails
                                    this.emails = [...this.emails, ...newEmails];
                                }
                                
                            // Update pagination state using backend response
                            if (data.pagination) {
                                this.hasMoreEmails = data.pagination.has_more;
                                this.emailOffset = data.pagination.current_offset + data.pagination.loaded_count;
                            } else {
                                // Fallback logic
                                this.hasMoreEmails = data.emails.length >= 25;
                                this.emailOffset += data.emails.length;
                            }
                            } else {
                ...
                loadMoreEmails() {
                    if (!this.hasMoreEmails || this.isLoadingMore || this.isLoading) {
                        return;
                    }

                    this.isLoadingMore = true;
                    
                    // Call loadEmails but it will append to existing emails due to offset > 0
                    this.loadEmails();
                },

                resetEmailPagination() {
                    this.emailOffset = 0;
                    this.hasMoreEmails = true;
                    this.isLoadingMore = false;
                },

                checkScrollForLoadMore(event) {
                ...
                syncEmails() {
                    ...
                    // Use paginated sync for better reliability
                    this.syncEmailsWithPagination();
                },
                syncAllFolders() {
                    if (!this.selectedAccountId) {
                        alert('Please select an email account first.');
                    ...
                    this.syncDetails = '';
                    let queryParams = `folder=all&limit=5`;
                    ...
                    fetch(`/email_users/emails/sync/${this.selectedAccountId}?${queryParams}`, {
                        method: 'POST',
                        headers: {
                ...
                loadSignatures() {
                    ...
                    fetch(`/email_users/signatures/account/${this.selectedAccountId}`)
                        .then(r => r.json())
                        .then(res => {
                            if (res.success) {
                ...
                allocateForm: { clientId: '', matterId: '' },
                clients: [],
                clientMatters: [],
                deleteEmail(email) {
                ...
                validateDateRange() {
                    if (this.startDate && this.endDate) {
                        ...
                        if (diffDays > 5) {
                            alert('Maximum date range allowed is 5 days. Please select a smaller range.');
                            this.endDate = '';
                            return false;
                        }
                    }
                ...
                async syncEmailsWithPagination() {
                    // Show confirmation dialog
                    let confirmMessage = `Sync ALL emails from ${this.selectedFolder} folder?`;
                    if (this.startDate && this.endDate) {
                        confirmMessage = `Sync ALL emails from ${this.selectedFolder} folder between ${this.startDate} and ${this.endDate}?`;
                    } else if (this.startDate) {
                        confirmMessage = `Sync ALL emails from ${this.selectedFolder} folder from ${this.startDate} onwards?`;
                    } else if (this.endDate) {
                        confirmMessage = `Sync ALL emails from ${this.selectedFolder} folder until ${this.endDate}?`;
                    }
                    
                    if (!confirm(confirmMessage + '\n\nThis will sync in small batches to prevent timeouts and ensure no emails are missed.')) {
                        return;
                    }

                    // Initialize sync state
                    this.showSyncModal = true;
                    this.syncProgressText = 'Starting paginated sync...';
                    this.syncDetails = '';
                    ...
                    try {
                        while (hasMore) {
                            this.syncProgressText = `Syncing batch ${batchNumber} (offset: ${offset})...`;
                            
                            // Build query parameters for this batch
                            let queryParams = `folder=${this.selectedFolder}&paginated=true&offset=${offset}`;
                            if (this.startDate) {
                            ...
                            const response = await fetch(`/email_users/emails/sync/${this.selectedAccountId}?${queryParams}`, {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                            ...
                            // Update counters
                            totalSynced += data.newCount || 0;
                            totalDuplicates += data.duplicateCount || 0;
                            hasMore = data.hasMore || false;
                            offset = data.nextOffset || (offset + 10);

                            // Update progress
                            this.syncDetails = `Batch ${batchNumber}: ${data.newCount || 0} new, ${data.duplicateCount || 0} duplicates`;
                            if (hasMore) {
                                this.syncDetails += ` | Total so far: ${totalSynced} new emails`;
                            }
                            ...
                            if (batchNumber > 50) {
                                console.warn('Stopped sync after 50 batches for safety');
                                break;
                            }
                        }

                        // Final success message
                        this.syncProgressText = 'Sync completed successfully!';
                        this.syncDetails = `Total: ${totalSynced} new emails synced, ${totalDuplicates} duplicates skipped in ${batchNumber - 1} batches`;
                        ...
                        console.error('Paginated sync error:', error);
                        this.syncProgressText = 'Sync failed';
                        this.syncDetails = error.message || 'An error occurred during sync';
                    }
                },
                
                applyFilters() {
                    this.addToRecentSearches(this.searchQuery);
                    this.resetEmailPagination(); // Reset pagination when applying filters
                    this.loadEmails();
                ...
                loadEmailContent(emailId) {
                    ...
                    fetch(`/email_users/emails/content/${emailId}`)
                        .then(response => response.json())
                        .then(data => {
                ...
                bulkMarkAsRead() {
                    ...
                    fetch('/email_users/emails/bulk-action', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                ...
                bulkFlag() {
                    ...
                    .then(data => {
                        if (data.success) {
                            // Update emails with complete updated data if available
                            if (data.updated_emails && data.updated_emails.length > 0) {
                                data.updated_emails.forEach(updatedEmail => {
                                    const emailIndex = this.emails.findIndex(email => email.id === updatedEmail.id);
                                    if (emailIndex !== -1) {
                                        // Update the email with complete updated data including flag status
                                        this.emails[emailIndex] = updatedEmail;
                                    }
                                });
                            } else {
                                // Fallback: Update local email data with basic flag status
                                this.emails.forEach(email => {
                                    if (this.selectedEmails.includes(email.id)) {
                ...
                // Email allocation methods
                openAllocateModal() {
                    if (this.selectedEmails.length === 0) {
                        alert('Please select at least one email to allocate.');
                        return;
                    }
                ...
                initializeSelect2() {
                    ...
                    if ($('#matterSelect').hasClass('select2-hidden-accessible')) {
                        $('#matterSelect').select2('destroy');
                    }
                    ...
                    $('#clientSelect').select2({
                        ...
                        allowClear: true,
                        width: '100%',
                        dropdownParent: $('#clientSelect').closest('.fixed.inset-0')
                    });

                    // Initialize matter Select2
                    $('#matterSelect').select2({
                        placeholder: 'Select matter...',
                        allowClear: true,
                        width: '100%',
                        dropdownParent: $('#matterSelect').closest('.fixed.inset-0')
                    });

                    // Handle client selection change
                    $('#clientSelect').off('change').on('change', (e) => {
                        this.allocateForm.clientId = e.target.value;
                        this.loadClientMatters();
                    });

                    // Handle matter selection change
                    $('#matterSelect').off('change').on('change', (e) => {
                        this.allocateForm.matterId = e.target.value;
                    });
                },

                loadClients() {
                    fetch('/email_users/clients')
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                this.clients = data.clients;
                                // Refresh Select2 after data is loaded
                                this.$nextTick(() => {
                                    if ($('#clientSelect').hasClass('select2-hidden-accessible')) {
                                        $('#clientSelect').select2('destroy');
                                    }
                ...
                loadClientMatters() {
                    if (!this.allocateForm.clientId) {
                        this.clientMatters = [];
                        this.allocateForm.matterId = '';
                        // Clear matter Select2
                        this.$nextTick(() => {
                            if ($('#matterSelect').hasClass('select2-hidden-accessible')) {
                                $('#matterSelect').select2('destroy');
                            }
                            $('#matterSelect').empty().append('<option value="">Select matter...</option>');
                            $('#matterSelect').select2({
                                placeholder: 'Select matter...',
                                allowClear: true,
                                width: '100%',
                    ...
                    fetch(`/email_users/client-matters/${this.allocateForm.clientId}`)
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                this.clientMatters = data.matters;
                                // Refresh matter Select2 after data is loaded
                                this.$nextTick(() => {
                                    if ($('#matterSelect').hasClass('select2-hidden-accessible')) {
                                        $('#matterSelect').select2('destroy');
                                    }
                                ...
                                alert('Error loading matters: ' + data.message);
                                this.clientMatters = [];
                            }
                        })
                        .catch(error => {
                            console.error('Error loading matters:', error);
                            alert('An error occurred while loading matters.');
                            this.clientMatters = [];
                        });
                },
                ...
                allocateEmails() {
                    ...
                    const matterId = $('#matterSelect').val();

                    if (!clientId || !matterId) {
                        alert('Please select both client and matter.');
                        return;
                    }
                    ...
                    fetch('/email_users/emails/allocate', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                    ...
                    .then(data => {
                        if (data.success) {
                            alert('Emails allocated successfully.');
                            this.closeAllocateModal();
                            
                            // Update the allocated emails with their new client/matter data
                            if (data.updated_emails && data.updated_emails.length > 0) {
                                data.updated_emails.forEach(updatedEmail => {
                                    const emailIndex = this.emails.findIndex(email => email.id === updatedEmail.id);
                                    if (emailIndex !== -1) {
                                        // Update the email with new allocation data
                                        this.emails[emailIndex] = updatedEmail;
                                    }
                                });
                            } else {
                                // Fallback: refresh the email list if updated data not available
                                this.loadEmails();
                            }
                        } else {
                            alert('Error allocating emails: ' + data.message);
                        }
                    })
                    .catch(error => {
                        console.error('Error allocating emails:', error);
                        alert('An error occurred while allocating emails.');
                    });
                },
                ...
                closeAllocateModal() {
                    ...
                    this.allocateForm = { clientId: '', matterId: '' };
                    this.clientMatters = [];
                    this.selectedEmails = [];
                },
