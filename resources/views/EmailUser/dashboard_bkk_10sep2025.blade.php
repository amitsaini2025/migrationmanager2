@extends('layouts.EmailUser.app_outlook')
...
@section('content')
            ...
            <div class="bg-white border border-gray-200 sm:rounded-lg">
                        ...
                        <div class="flex gap-3 items-center">
                            <div>
                                ...
                                <select id="accountDropdown" x-model="selectedAccountId" @change="loadEmails" class="rounded-md border-gray-300 text-sm">
                                    ...
                                    <template x-for="acct in accounts" :key="acct?.id || Math.random()">
                                        <option :value="acct?.id || ''" x-text="acct?.label || 'Unknown Account'"></option>
                                    </template>
                                </select>
                            </div>
                            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 h-[70vh]">
                            ...
                            <template x-for="group in groupedEmails" :key="group.date">
                                <div>
                                    ...
                                    <div class="divide-y divide-gray-100">
                                        <template x-for="email in group.emails" :key="email?.id || Math.random()">
                                            <button type="button"
                    ...
                    <div class="md:col-span-2 h-full overflow-y-auto bg-white">
                                ...
                                <div class="flex-1 overflow-y-auto">
                                    <div class="p-6">
                                        ...
                                        <div x-show="selectedEmail.attachments && selectedEmail.attachments.length > 0" class="mt-6 border-t border-gray-200 pt-6">
                                            ...
                                            <div class="space-y-3">
                                                <template x-for="attachment in selectedEmail.attachments" :key="attachment?.id || attachment?.name || Math.random()">
                        ...
                        <div x-show="!selectedEmail" class="h-full flex items-center justify-center text-gray-500">
                            <div class="text-center">
                                ...
                                <p class="text-sm text-gray-500">Select an email from the list to view its content.</p>
                            </div>
                        </div>
                        </div>
                    </div>
    ...
    <div x-show="showAttachmentPreview && previewAttachment && previewAttachment?.id && previewAttachment.id !== 'undefined' && previewAttachment.id !== null" 
         x-cloak
         class="fixed inset-0 z-50 overflow-y-auto" 
         :style="(showAttachmentPreview && previewAttachment && previewAttachment?.id && previewAttachment.id !== 'undefined' && previewAttachment.id !== null) ? 'display: block; background-color: rgba(0, 0, 0, 0.5);' : 'display: none;'"
         x-transition>
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] flex flex-col" @click.stop>
                <div class="flex items-center justify-between p-6 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900" x-text="previewAttachment ? (previewAttachment?.name || previewAttachment?.filename || 'Attachment') : ''"></h3>
                    <button type="button" class="text-gray-400 hover:text-gray-600" @click="showAttachmentPreview = false">
                ...
                <div class="flex-1 overflow-hidden">
                    <div class="flex items-center justify-center h-full min-h-[400px]">
                        <div x-show="previewAttachment?.type && previewAttachment.type.startsWith('image/')" class="text-center">
                            <img :src="`/email_users/attachments/${previewAttachment.id}/view`" 
                                 :alt="previewAttachment?.name || previewAttachment?.filename || 'Attachment'"
                                 class="max-h-[70vh] max-w-full object-contain">
                        </div>
                        <div x-show="previewAttachment?.type && previewAttachment.type.includes('pdf')" class="w-full h-full">
                            <iframe :src="`/email_users/attachments/${previewAttachment.id}/view#toolbar=0&navpanes=0&scrollbar=0`" 
                                    class="w-full h-[70vh] border-0"
                                    :title="previewAttachment?.name || previewAttachment?.filename || 'Attachment'">
                            </iframe>
                        </div>
                        <div x-show="!previewAttachment?.type || (!previewAttachment.type.startsWith('image/') && !previewAttachment.type.includes('pdf'))" class="text-center text-gray-500">
                            <svg class="w-16 h-16 mx-auto mb-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                            </svg>
                            <p class="mb-4">Preview not available for this file type.</p>
                            <button class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" @click="downloadAttachment(previewAttachment)">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center justify-end gap-3 p-6 border-t border-gray-200">
                    <button type="button" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" @click="showAttachmentPreview = false">Close</button>
                    <button type="button" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" @click="downloadAttachment(previewAttachment)">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                        </svg>
                        Download
    ...
    <div x-show="showCompose" 
         x-cloak
         class="fixed inset-0 z-50 overflow-y-auto" 
         :style="showCompose ? 'display: block; background-color: rgba(0, 0, 0, 0.5);' : 'display: none;'"
         x-transition>
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col" @click.stop>
                <div class="flex items-center justify-between p-6 border-b border-gray-200">
                    ...
                    <button type="button" class="text-gray-400 hover:text-gray-600" @click="showCompose = false">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </button>
                ...
                <div class="flex-1 overflow-y-auto p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select x-model="selectedAccountId" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                                <option value="">Select an account</option>
                                <template x-for="acct in accounts" :key="acct?.id || Math.random()">
                                    <option :value="acct?.id || ''" x-text="acct?.label || 'Unknown Account'"></option>
                                </template>
                        ...
                        <div>
                            ...
                            <input type="email" x-model="compose.to" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" placeholder="recipient@example.com" />
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Cc</label>
                            <input type="text" x-model="compose.cc" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Bcc</label>
                            <input type="text" x-model="compose.bcc" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" />
                        </div>
                    </div>
                    <div class="mt-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Subject</label>
                        <input type="text" x-model="compose.subject" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" />
                    </div>
                    <div class="mt-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Body</label>
                        <textarea rows="8" x-model="compose.body" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Type your message here..."></textarea>
                    </div>
                    <div class="mt-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Attachments</label>
                        <input type="file" x-ref="attachmentInput" @change="handleFileSelection" multiple class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" accept="*/*">
                        <div class="mt-2">
                            <template x-for="(file, index) in compose.attachments" :key="index">
                                <div class="flex justify-between items-center p-2 bg-gray-50 rounded border mb-2">
                                    <div class="flex items-center">
                                        <svg class="w-4 h-4 text-gray-400 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M8 4a3 3 0 00-3 3v4a5 5 0 0010 0V7a1 1 0 112 0v4a7 7 0 11-14 0V7a5 5 0 0110 0v4a3 3 0 11-6 0V7a1 1 0 012 0v4a1 1 0 102 0V7a3 3 0 00-3-3z" clip-rule="evenodd"></path>
                                        </svg>
                                        <span class="text-sm" x-text="file?.name || 'Unknown file'"></span>
                                        <span class="text-xs text-gray-500 ml-2" x-text="formatFileSize(file.size)"></span>
                                    </div>
                                    <button type="button" @click="removeAttachment(index)" class="p-1 hover:bg-gray-200 rounded">
                ...
                <div class="flex items-center justify-end gap-3 p-6 border-t border-gray-200">
                    <button type="button" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" @click="showCompose = false">Cancel</button>
                    <button type="button" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" @click="sendEmail({ to: compose.to, cc: compose.cc, bcc: compose.bcc, subject: compose.subject, body: compose.body })">Send</button>
                </div>
            </div>
        </div>
    </div>
        ...
        function emailApp() {
            return {
                ...
                selectEmail(email) {
                    ...
                    // Load email content on-demand if not already loaded
                    if ((!email.body || email.body === '') && (!email.htmlBody || email.htmlBody === '')) {
                        this.loadEmailContent(email.id);
                    }
                },
                loadEmailContent(emailId) {
                    fetch(`/email_users/emails/${emailId}/content`)
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                // Update the selected email with the loaded content
                                if (this.selectedEmail && this.selectedEmail.id === emailId) {
                                    this.selectedEmail.body = data.body;
                                    this.selectedEmail.htmlBody = data.html_body;
                                    
                                    // Also update the email in the emails array
                                    const emailIndex = this.emails.findIndex(e => e.id === emailId);
                                    if (emailIndex !== -1) {
                                        this.emails[emailIndex].body = data.body;
                                        this.emails[emailIndex].htmlBody = data.html_body;
                                    }
                                }
                            } else {
                                ...
                                if (this.selectedEmail && this.selectedEmail.id === emailId) {
                                    this.selectedEmail.body = data.message || 'No content available for this email.';
                                    this.selectedEmail.htmlBody = '';
                                }
                            }
                        })
                        .catch(error => {
                            ...
                            // Show user-friendly error message
                            if (this.selectedEmail && this.selectedEmail.id === emailId) {
                                this.selectedEmail.body = 'Error loading email content. Please try again.';
                                this.selectedEmail.htmlBody = '';
                            }
                        });
                },
                ...
                downloadAttachment(attachment) {
                    if (attachment?.id && attachment.id !== 'undefined' && attachment.id !== null) {
                        ...
                        link.href = `/email_users/attachments/${attachment.id}/download`;
                        link.download = attachment?.filename || attachment?.name || 'attachment';
                        document.body.appendChild(link);
                        link.click();
                        document.body.removeChild(link);
                    } else {
                        console.error('Attachment ID not found or invalid:', attachment);
                        alert('Unable to download attachment: Invalid attachment data');
                    }
                },
                viewAttachment(attachment) {
                    if (attachment?.id && attachment.id !== 'undefined' && attachment.id !== null) {
                        ...
                        console.error('Attachment ID not found or invalid:', attachment);
                        alert('Unable to view attachment: Invalid attachment data');
                    }
                },
