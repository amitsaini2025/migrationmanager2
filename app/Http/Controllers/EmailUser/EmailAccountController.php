...
class EmailAccountController extends Controller
{
    ...
    {
        $accounts = EmailAccount::where('user_id', Auth::guard('email_users')->id())
            ->orderBy('created_at', 'desc')
            ->get();

        return view('EmailUser.accounts.index', compact('accounts'));
    }
    ...
    {
        return view('EmailUser.accounts.create');
    }
    ...
    public function store(StoreEmailAccountRequest $request)
    {
        try {
            $account = EmailAccount::create([
                'user_id' => Auth::guard('email_users')->id(),
                'provider' => 'zoho',
                'email' => $request->email,
            ...
            return redirect()->route('email_users.accounts.index')
    ...
    public function show(EmailAccount $account)
    {
        ...
        return view('EmailUser.accounts.show', compact('account'));
    }
    ...
    public function edit(EmailAccount $account)
    {
        ...
        return view('EmailUser.accounts.edit', compact('account'));
    }
    ...
    public function testConnection(EmailAccount $account)
    {
        ...
        try {
            ...
            $python = base_path('python_outlook_web/venv/Scripts/python.exe'); // Use virtual environment Python
            if (strtoupper(substr(PHP_OS, 0, 3)) !== 'WIN') {
                $python = base_path('python_outlook_web/venv/bin/python'); // Use virtual environment Python on Unix-like systems
            }

            $script = base_path('python_outlook_web/test_network.py');
    ...
    public function testAuthentication(EmailAccount $account)
    {
        ...
        try {
            ...
            $script = base_path('python_outlook_web/sync_emails.py');
