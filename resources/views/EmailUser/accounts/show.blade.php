<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('email_users.accounts.edit', $account) }}" 
                                                ...
                                                @foreach($attachments as $att)
                                                    <div class="flex items-center justify-between p-2 bg-gray-50 rounded border">
                                                        ...
                                                        <div class="flex items-center space-x-1">
                                                            @if($att->canPreview())
                                                                <a href="{{ route('email_users.attachments.view', $att->id) }}" target="_blank" class="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded hover:bg-blue-200" title="View">
                                                                    View
                                                                </a>
                                                            @endif
                                                            <a href="{{ route('email_users.attachments.download', $att->id) }}" class="px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded hover:bg-gray-200" title="Download">
                                                                Download
                                                            </a>
                                                        </div>
                                                    </div>
    ...
    <script>
        function testConnection() {
            ...
            fetch(`/email_users/accounts/{{ $account->id }}/test-connection`, {
                method: 'POST',
                headers: {
        ...
        function testAuthentication() {
            ...
            fetch(`/email_users/accounts/{{ $account->id }}/test-authentication`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Content-Type': 'application/json',
        ...
        function syncEmails() {
            ...
            fetch(`/email_users/emails/sync/{{ $account->id }}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Content-Type': 'application/json',
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            try {
                const res = await fetch('{{ route('email_users.labels.index') }}');
        ...
        async function applyLabel(emailId) {
            ...
            if (!labelId) return;
            const res = await fetch('{{ route('email_users.labels.apply') }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Content-Type': 'application/json',
